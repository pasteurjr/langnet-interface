...
interface SpecificationHistoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId?: string;
...
const SpecificationHistoryModal: React.FC<SpecificationHistoryModalProps> = ({
  isOpen,
  onClose,
  projectId,
  onSelectSession,
  ...
  const [sessions, setSessions] = useState<SpecificationSession[]>([]);
  ...
  const loadSessions = async () => {
    ...
    setError(null);
    try {
      let url = `${API_BASE_URL}/specifications?limit=50&offset=0`;
      if (projectId) {
      ...
      if (!response.ok) {
        throw new Error('Failed to load specification sessions');
      }

      const data = await response.json();
      setSessions(data.sessions || []);
    } catch (err) {
      console.error('Error loading specification sessions:', err);
      setError('Erro ao carregar histórico de especificações');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadVersions = async (sessId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/specifications/${sessId}/versions`, {
        method: 'GET',
        headers: getAuthHeaders()
      });

      if (!response.ok) {
        throw new Error('Failed to load specification versions');
      }

      const data = await response.json();
      setVersions(data.versions || []);
    } catch (err) {
      console.error('Error loading specification versions:', err);
  ...
  const handleSessionClick = (session: SpecificationSession) => {
    ...
    <div className="modal-overlay">
      <div className="history-modal">
        <div className="modal-header">
          <h2>{viewMode === 'sessions' ? '📜 Histórico de Especificações' : '📜 Histórico de Versões'}</h2>
        ...
        {viewMode === 'versions' && canGoBack && (
          <div style={{
            ...
            >
              ← Voltar para Lista de Especificações
            </button>
          </div>
        )}
        ...
        <div className="modal-content">
          {loading && (
            <div className="loading-state">
              ...
              <p>{viewMode === 'sessions' ? 'Carregando especificações...' : 'Carregando versões...'}</p>
            </div>
          )}
          ...
          {!loading && !error && viewMode === 'sessions' && sessions.length === 0 && (
            <div className="empty-state">
              ...
              <p>Nenhuma especificação gerada ainda</p>
              <p className="empty-hint">
                As especificações funcionais geradas aparecerão aqui
              </p>
            </div>
          )}
                ...
                >
                  ...
                  <div className="session-details">
                    ...
                    <div className="session-info">
                      ...
                      <span className="info-value">Requisitos v{session.requirements_version}</span>
                    </div>

                    {session.total_sections && (
                      <div className="session-info">
                        <span className="info-label">Seções:</span>
                        <span className="info-value">{session.total_sections}</span>
                      </div>
                    )}
                  </div>
          ...
          {!loading && !error && viewMode === 'versions' && versions.length === 0 && (
            <div className="empty-state">
              ...
              <p className="empty-hint">
                As versões da especificação aparecerão aqui
              </p>
            </div>
          )}
          ...
          <div className="footer-info">
            {!loading && viewMode === 'sessions' && sessions.length > 0 && (
              <span className="sessions-count">
                {sessions.length} especificação(ões) encontrada(s)
              </span>
            )}
