...
import PetriPythonConverter from "./PetriPythonConverter";
...
const Modal = ({ isOpen, onClose, title, children, size = "medium" }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ marginBottom: "20px", color: "#333333" }}>{title}</h2>
        <button
          onClick={onClose}
          style={{
            position: "absolute",
    ...
    >
      <div>
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          <div>
            <label style={{ color: "#333333" }}>Nome: </label>
            <input
              type="text"
              name="nome"
              value={formData.nome || ""}
        ...
        >
          <button onClick={onClose} style={{ color: "#333333", backgroundColor: "#f5f5f5", border: "1px solid #ccc", padding: "8px 16px" }}>Cancelar</button>
          <button onClick={() => onSave(formData)} style={{ color: "#fff", backgroundColor: "#1976d2", border: "none", padding: "8px 16px" }}>Salvar</button>
        </div>
      </div>
    </Modal>
  );
 ...
 */
      ...
      >
        ...
        <div
          style={{
            padding: "8px 12px",
            cursor: "pointer",
            color: "#d32f2f", // Texto vermelho para excluir
  ...
  const createField = (label, name, type, options = {}) => (
    <div>
      <label style={{ color: "#333333" }}>{label}: </label>
      <input
        type={type}
        name={name}
      ...
      case "place":
        return (
          ...
          >
            ...
            <div style={{ padding: "10px", backgroundColor: "#f5f5f5", borderRadius: "4px", color: "#333333" }}>
              <small>
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Nome da Rede">
      <div>
        ...
        >
          <button 
            onClick={onClose}
            style={{ color: "#333333", backgroundColor: "#f5f5f5", border: "1px solid #ccc", padding: "8px 16px" }}
          >
            Cancelar
          </button>
          <button 
            onClick={() => onSave(name)}
            style={{ color: "#fff", backgroundColor: "#1976d2", border: "none", padding: "8px 16px" }}
          >
            Confirmar
          </button>
        </div>
...
const PetriNetEditor = forwardRef(({ externalData = null, onDataLoad = null, compactMode = false, onSimulationStart = null, onPlaceProcessed = null, pausedByTaskUI = false, suppressVerbosePanel = false }, ref) => {
  // Refs
 ...
 */
  ...
  const handleStartSimulation = () => {
    try {
      ...
      // ✨ CONFIGURAR CALLBACK onVerboseStep PARA VERBOSEPANEL ANTES DE INICIAR
      newSimulator.onVerboseStep = handleVerboseStep;
      
      // ✨ CONFIGURAR CALLBACK NO PLACEPROCESSOR TAMBÉM (DIRETO)
      newSimulator.placeProcessor.setCallbacks({
        ...newSimulator.placeProcessor.callbacks,
        onVerboseStep: handleVerboseStep,
        onPlaceProcessed: (placeId, inputData, outputData) => {
          try {
            if (typeof onPlaceProcessed === 'function') {
              onPlaceProcessed({ placeId, inputData, outputData });
            }
          } catch (e) {
            console.warn('onPlaceProcessed prop handler lançou exceção:', e);
          }
        }
      });
      console.log('🎯 Callback onVerboseStep configurado no simulator E placeProcessor');
  ...
  // Expor método imperativo para o pai iniciar simulação automática via painel
  useImperativeHandle(ref, () => ({
    startAutoSimulation() {
      try {
        // ✅ Proteção: só inicia se a rede estiver carregada
        const netReady = petriNet && Array.isArray(petriNet.lugares) && petriNet.lugares.length > 0 && Array.isArray(petriNet.transicoes) && petriNet.transicoes.length > 0;
        if (!netReady) {
          console.warn('⏳ Rede ainda não carregada; reagendando start em 200ms');
          setTimeout(() => {
            try { ref && ref.current && ref.current.startAutoSimulation && ref.current.startAutoSimulation(); } catch (e) {}
          }, 200);
          return;
        }
        // Usa exatamente o mesmo fluxo do botão "Simular"
        handleStartSimulation();
        // Após abrir o painel, aciona o modo automático do próprio SimulationPanel
        setTimeout(() => {
          if (simulationPanelRef.current && typeof simulationPanelRef.current.startAuto === 'function') {
            simulationPanelRef.current.startAuto();
          }
        }, 200);
      } catch (e) {
        console.error('Erro ao iniciar simulação automática via ref:', e);
      }
    }
    ,
    pauseAutoSimulation() {
      try {
        if (simulationPanelRef.current && typeof simulationPanelRef.current.pauseAuto === 'function') {
          simulationPanelRef.current.pauseAuto();
        }
      } catch (e) {
        console.error('Erro ao pausar simulação automática via ref:', e);
      }
    }
    ,
    resumeAutoSimulation() {
      try {
        if (simulationPanelRef.current && typeof simulationPanelRef.current.resumeAuto === 'function') {
          simulationPanelRef.current.resumeAuto();
        }
      } catch (e) {
        console.error('Erro ao retomar simulação automática via ref:', e);
      }
    }
  }));
  ...
  // REMOVIDO: WebSocket state (não é mais necessário com CentralWSClient)
  // const [webSocketClient, setWebSocketClient] = useState(null);
  // const [wsConnected, setWsConnected] = useState(false);
  
  const MAX_HISTORY = 50;

  // =========== VERBOSE FUNCTIONS (WebSocket removido - usando CentralWSClient) ===========

  // REMOVIDO: connectWebSocket() 
  // O VerbosePanel agora usa CentralWSClient diretamente através da infraestrutura FakeWebSocket
  // Conexões WebSocket são gerenciadas pela infraestrutura: FakeWebSocket → CentralWSClient → VerbosePanel

  const handleVerboseStep = (data) => {
    console.log('🔥 DEBUG: handleVerboseStep - INÍCIO');
    console.log('🔥 DEBUG: data recebido:', data);
    console.log('🔥 DEBUG: verbosePanelVisible:', verbosePanelVisible);
    console.log('🔥 DEBUG: mdContent atual comprimento:', mdContent.length);

    if (!data) {
      console.log('❌ DEBUG: data é null/undefined');
      return;
    }

    // SEMPRE processar dados (remover verificação verbosePanelVisible)
    console.log('✅ DEBUG: Processando data...');

    // Atualizar contadores de tarefas
    if (data.task_name && taskCounters[data.task_name]) {
      console.log('🔥 DEBUG: Atualizando contadores para:', data.task_name);
      setTaskCounters(prev => ({
        ...prev,
        [data.task_name]: {
          ...prev[data.task_name],
          [data.section_type]: (prev[data.task_name][data.section_type] || 0) + 1
        }
      }));
    }
    ...
    // Verificar se precisa adicionar cabeçalho da tarefa
    const taskName = data.task_name;
    const taskHeader = `## 📋 TAREFA: ${taskName?.toUpperCase()}`;
    if (taskName && !mdContent.includes(taskHeader)) {
      console.log('🔥 DEBUG: Adicionando cabeçalho da tarefa:', taskName);
      setMdContent(prev => {
        const newContent = prev + `${taskHeader}\n**Status:** ⚡ Executando\n**Contadores:** Inputs: 0 | Steps: 0 | Outputs: 0\n\n`;
        console.log('🔥 DEBUG: Novo conteúdo com tarefa:', newContent.substring(newContent.length - 100));
        return newContent;
      });
    }

    // Verificar se já existe header da seção
    const sectionHeader = `### ${sectionMap[data.section_type] || 'LOGS'}`;
    if (!mdContent.includes(sectionHeader)) {
      console.log('🔥 DEBUG: Adicionando header da seção:', sectionHeader);
      setMdContent(prev => {
        const newContent = prev + `${sectionHeader}\n\n`;
        console.log('🔥 DEBUG: Novo conteúdo com seção:', newContent.substring(newContent.length - 50));
        return newContent;
      });
    }
    ...
    const newLine = `**[${timestamp}] [${type}]** ${content}\n\n`;
    
    console.log('🔥 DEBUG: Adicionando linha:', newLine);
    setMdContent(prev => {
      const newContent = prev + newLine;
      console.log('🔥 DEBUG: Conteúdo total final comprimento:', newContent.length);
      return newContent;
    });

    console.log('🔥 DEBUG: handleVerboseStep - FIM');
  };
  ...
  const calculateDiagramBounds = () => {
    ...
    if (elements.length === 0) {
      // Usar dimensões otimizadas para o tamanho do card
      const defaultWidth = 1000; // Largura próxima ao tamanho do card
      return { minX: 0, minY: 0, maxX: defaultWidth, maxY: 400, width: defaultWidth, height: 400 };
    }
  ...
  /**
   * Load data programmatically (same logic as handleLoad but for external data)
   */
  const loadDataProgrammatically = (jsonData) => {
    try {
      console.log('🔄 Carregando dados externos programaticamente:', jsonData);
      let loadedNet = jsonData;
      clearNetwork();
      // Garantir que a estrutura de agentes exista
      if (!loadedNet.agentes) {
        loadedNet.agentes = [];
      ...
      setCurrentFile({ name: "Dados externos" });
  ...
  useEffect(() => {
    ...
    if (!paperRef.current) {
      const containerElement = document.getElementById("paper-container");
      // Tamanho bom (1300px) + MAIS 100px = 1400px total
      const containerWidth = containerElement ? 
        Math.max(containerElement.offsetWidth - 20, 1200) + 200 : 1400;
      
      paperRef.current = new joint.dia.Paper({
        el: containerElement,
        model: graphRef.current,
        width: containerWidth, // Largura otimizada próxima ao card
        height: 400,           // Altura mantida
        gridSize: 10,
        interactive: {
  ...
  // UseEffect para resize responsivo do canvas
  useEffect(() => {
    const handleResize = () => {
      if (paperRef.current) {
        const containerElement = document.getElementById("paper-container");
        if (containerElement) {
          const newWidth = Math.max(containerElement.offsetWidth - 20, 1200) + 200; // +200px total (era 100, agora +100 = 200)
          paperRef.current.setDimensions(newWidth, 400);
        }
      }
    };
  ...
  /**
   * Load external data when provided via props
   */
  useEffect(() => {
    if (externalData) {
      console.log('📥 Dados externos recebidos via props, carregando...');
      // Aguardar um pouco para garantir que o paper foi inicializado
      setTimeout(() => {
        loadDataProgrammatically(externalData);
      }, 500);
    }
  }, [externalData]);
  ...
  return (
    <div
      style={{
        ...
        height: compactMode ? "500px" : "600px", // Altura ajustada para canvas maior
        width: "100%",
        maxWidth: "100%", // Garante que não exceda a largura do container pai
        padding: "10px",
        background: "white"
      }}
    >
      <div>
        <div style={{ textAlign: "center", marginBottom: "10px" }}>
          {compactMode ? (
            /* Toolbar Compacta - Apenas 3 botões essenciais */
            <div style={{ 
              display: "flex", 
              justifyContent: "center", 
              gap: "8px",
              ...
              <button
                ...
                onClick={() => {
                  console.log('🔍 COMPACT JSON Modal - petriNet:', petriNet);
                  console.log('🔍 COMPACT JSON Modal - externalData:', externalData);
            ...
            /* Toolbar Completa - Todos os botões */
            <>
              {/* File operations */}
              <button
              ...
              <button
                ...
                onClick={() => {
                  console.log('🔍 JSON Modal - petriNet:', petriNet);
                  console.log('🔍 JSON Modal - externalData:', externalData);
                  setJsonModalOpen(true);
                }}
              >
                📋 JSON
        ...
        <div
          ...
          style={{
            ...
            width: "100%",      // Container ocupa toda largura do card
            height: "400px",    // Altura mantida
            overflowX: "auto",  // Scroll horizontal para área maior
          ...
          >
            <pre
              style={{
                ...
                color: "#333333", // COR PRETA para o texto JSON!
                padding: "10px",
                borderRadius: "5px",
                margin: 0,
                fontSize: "12px"
        ...
        <SimulationPanel
          ...
          onSimulationStart={() => {
          // ✨ CALLBACK PARA ATIVAR VERBOSEPANEL - NO PETRI NET EDITOR!
          console.log('🔥 DEBUG: onSimulationStart do PetriNetEditor chamado!');
          console.log('🔥 DEBUG: Antes - verbosePanelVisible:', verbosePanelVisible);
          
          // Ativar VerbosePanel (se não suprimido pelo pai)
          if (!suppressVerbosePanel) {
            setVerbosePanelVisible(true);
            setVerboseSteps([]);
          }
            
            // REMOVIDO: Conexão WebSocket direta
            // O VerbosePanel usa CentralWSClient automaticamente através da infraestrutura FakeWebSocket
            console.log('🔌 VerbosePanel ativado - usando CentralWSClient via FakeWebSocket');
...
Iniciando execução via Simulador de Petri Net...`);
            ...
            // Propagar callback se existe
            if (onSimulationStart) {
              onSimulationStart();
            }
          }}
          pausedByTask={pausedByTaskUI}
        />
        {/* Context Menu */}
        ...
        {/* ✨ VerbosePanel - FLUTUANTE COMO SimulationPanel! */}
        {!suppressVerbosePanel && (
          <VerbosePanel
