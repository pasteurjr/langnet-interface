...
const PetriNetViewer = ({ visible, onClose, petriNetData, title }) => {
  // Refs
  ...
  // Initialize with data from props - IGUAL AO HANDLELOAD DO PETRINETEDITOR
  useEffect(() => {
    if (petriNetData && visible && graphRef.current) {
      try {
        console.log('🎯 PetriNetViewer carregando dados do project_data:', petriNetData);
        
        let loadedNet = petriNetData;
        
        // Limpar rede atual (igual clearNetwork)
        ...
        // Carregar todos os elementos (igual renderFullNetwork)
        ...
        console.log('✅ Petri Net carregada com sucesso!');
      } catch (error) {
        console.error('❌ Erro ao carregar Petri Net:', error);
      }
    }
  }, [petriNetData, visible]);
 ...
 */
  ...
  useEffect(() => {
    if (!paperRef.current || !visible) return;
  ...
  useEffect(() => {
    ...
    if (!paperRef.current) {
      paperRef.current = new joint.dia.Paper({
        el: visible ? document.getElementById("paper-container") : null,
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <h3 style={{ margin: 0, color: '#333' }}>{title || 'Petri Net Viewer'}</h3>
          <button 
            onClick={onClose}
            style={{
              background: 'none',
        ...
        {/* Conteúdo original do PetriNetEditor */}
        <div style={{ 
          flex: 1,
          padding: '16px',
          overflow: 'hidden'
      ...
      <div>
        ...
        <SimulationPanel
          ...
          onSimulationStart={() => {
            // ✨ CALLBACK PARA ATIVAR VERBOSEPANEL - EXATAMENTE O QUE O USUÁRIO PEDIU!
            console.log('🔥 DEBUG: onSimulationStart do PetriNetViewer chamado!');
            console.log('🔥 DEBUG: Antes - verbosePanelVisible:', verbosePanelVisible);
            setVerbosePanelVisible(true);
            console.log('🔥 DEBUG: setVerbosePanelVisible(true) executado!');
            setVerboseSteps([]);
...
Iniciando execução via Simulador de Petri Net...`);
        ...
        {/* ✨ VerbosePanel - EXATAMENTE ONDE O USUÁRIO PEDIU! */}
        {console.log('🔥 DEBUG: Render PetriNetViewer - verbosePanelVisible =', verbosePanelVisible)}
        {verbosePanelVisible && (
          <VerbosePanel
            ...
            isExecuting={true}
            isVisible={verbosePanelVisible}
            onToggleVisible={() => setVerbosePanelVisible(false)}
            position={{ x: 50, y: 100 }}
          />
        )}
      </div>
