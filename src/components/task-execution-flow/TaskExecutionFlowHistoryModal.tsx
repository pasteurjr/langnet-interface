...
interface TaskExecutionFlowSession {
  id: string;
  session_name: string;
  project_id: string;
  status: string;
...
interface TaskExecutionFlowVersion {
  version: number;
  created_at: string;
...
interface TaskExecutionFlowHistoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId?: string;
...
const TaskExecutionFlowHistoryModal: React.FC<TaskExecutionFlowHistoryModalProps> = ({
  isOpen,
  onClose,
  projectId,
  onSelectSession,
  ...
  const [sessions, setSessions] = useState<TaskExecutionFlowSession[]>([]);
  const [versions, setVersions] = useState<TaskExecutionFlowVersion[]>([]);
  ...
  const loadSessions = async () => {
    ...
    setError(null);
    try {
      let url = `${API_BASE_URL}/task-execution-flow?limit=50&offset=0`;
      if (projectId) {
      ...
      if (!response.ok) {
        throw new Error('Failed to load task execution flow sessions');
      }

      const data = await response.json();
      setSessions(data.sessions || []);
    } catch (err) {
      console.error('Error loading task execution flow sessions:', err);
      setError('Erro ao carregar histórico de fluxos de tarefas');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadVersions = async (sessId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/task-execution-flow/${sessId}/versions`, {
        method: 'GET',
        headers: getAuthHeaders()
      });

      if (!response.ok) {
        throw new Error('Failed to load task execution flow versions');
      }

      const data = await response.json();
      setVersions(data.versions || []);
    } catch (err) {
      console.error('Error loading task execution flow versions:', err);
  ...
  const getTypeLabel = (type: string) => {
    const labels: { [key: string]: string } = {
      ...
      'approval_revision': 'Revisão Aprovada',
  ...
  const getTypeBadge = (type: string) => {
    const typeColors: { [key: string]: string } = {
      ...
      'approval_revision': 'type-analysis',
      'feedback_incorporation': 'type-refinement'
    };
  ...
  const handleSessionClick = (session: TaskExecutionFlowSession) => {
    ...
    <div className="modal-overlay">
      <div className="history-modal">
        <div className="modal-header">
          <h2>{viewMode === 'sessions' ? '🔄 Histórico de Fluxos de Tarefas' : '📜 Histórico de Versões'}</h2>
        ...
        <div className="modal-content">
          {loading && (
            <div className="loading-state">
              ...
              <p>{viewMode === 'sessions' ? 'Carregando fluxos de tarefas...' : 'Carregando versões...'}</p>
            </div>
          )}
          ...
          {!loading && !error && viewMode === 'sessions' && sessions.length === 0 && (
            <div className="empty-state">
              ...
              <p>Nenhum fluxo de tarefas gerado ainda</p>
              <p className="empty-hint">
                Os fluxos de execução de tarefas gerados aparecerão aqui
              </p>
            </div>
          )}
              ...
              {sessions.map((session) => (
                ...
                >
                  ...
                  <div className="session-details">
                    ...
                    {session.total_tasks && (
                      <div className="session-info">
                        <span className="info-label">Total de Tarefas:</span>
                        <span className="info-value">{session.total_tasks}</span>
                      </div>
                    )}

                    {session.has_parallelism !== undefined && (
                      <div className="session-info">
                        <span className="info-label">Paralelismo:</span>
                        <span className="info-value">{session.has_parallelism ? '✓ Sim' : '✗ Não'}</span>
                      </div>
                    )}
                  </div>
          ...
          {!loading && !error && viewMode === 'versions' && versions.length === 0 && (
            <div className="empty-state">
              ...
              <p className="empty-hint">
                As versões do fluxo de tarefas aparecerão aqui
              </p>
            </div>
          )}
          ...
          <div className="footer-info">
            {!loading && viewMode === 'sessions' && sessions.length > 0 && (
              <span className="sessions-count">
                {sessions.length} fluxo(s) de tarefas encontrado(s)
              </span>
            )}
...
export default TaskExecutionFlowHistoryModal;
