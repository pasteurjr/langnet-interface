...
const AgentsYamlHistoryModal: React.FC<AgentsYamlHistoryModalProps> = ({
  isOpen,
  onClose,
  projectId,
  onSelectSession,
  ...
  const [sessions, setSessions] = useState<AgentsYamlSession[]>([]);
  const [versions, setVersions] = useState<AgentsYamlVersion[]>([]);
  ...
  const loadSessions = async () => {
    ...
    setError(null);
    try {
      let url = `${API_BASE_URL}/agents-yaml?limit=50&offset=0`;
      if (projectId) {
      ...
      setError('Erro ao carregar histórico de agents.yaml');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadVersions = async (sessId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/agents-yaml/${sessId}/versions`, {
        method: 'GET',
        headers: getAuthHeaders()
      });
  ...
  const handleSessionClick = (session: AgentsYamlSession) => {
    ...
    <div className="modal-overlay">
      <div className="history-modal">
        <div className="modal-header">
          <h2>{viewMode === 'sessions' ? '📜 Histórico de agents.yaml' : '📜 Histórico de Versões'}</h2>
        ...
        {viewMode === 'versions' && canGoBack && (
          <div style={{
            ...
            >
              ← Voltar para Lista de agents.yaml
            </button>
          </div>
        )}
        ...
        <div className="modal-content">
          {loading && (
            <div className="loading-state">
              ...
              <p>{viewMode === 'sessions' ? 'Carregando agents.yaml...' : 'Carregando versões...'}</p>
            </div>
          )}
          ...
          {!loading && !error && viewMode === 'sessions' && sessions.length === 0 && (
            <div className="empty-state">
              ...
              <p>Nenhum agents.yaml gerado ainda</p>
              <p className="empty-hint">
                Os arquivos agents.yaml gerados aparecerão aqui
              </p>
            </div>
          )}
                ...
                >
                  ...
                  <div className="session-details">
                    ...
                    <div className="session-info">
                      ...
                      <span className="info-value">Espec Agentes/Tarefas v{session.spec_version}</span>
                    </div>

                    {session.total_agents !== undefined && (
                      <div className="session-info">
                        <span className="info-label">Agentes:</span>
                        <span className="info-value">{session.total_agents}</span>
                      </div>
                    )}
                  </div>
          ...
          {!loading && !error && viewMode === 'versions' && versions.length === 0 && (
            <div className="empty-state">
              ...
              <p className="empty-hint">
                As versões do agents.yaml aparecerão aqui
              </p>
            </div>
          )}
          ...
          <div className="footer-info">
            {!loading && viewMode === 'sessions' && sessions.length > 0 && (
              <span className="sessions-count">
                {sessions.length} agents.yaml encontrado(s)
              </span>
            )}
...
export default AgentsYamlHistoryModal;
