...
(async()=>{
  ...
  // INSTRUÇÃO DA ETAPA: a interface tem de realizar o que os casos de uso descrevem — os
  // campos, os botões e as mensagens que cada UC nomeia, não um formulário genérico. Foi
  // cobrado pelo user em 19/09/2026.
  const INSTR = 'Cada tela deve realizar o caso de uso correspondente da especificação: os ' +
    'campos, os botões e as mensagens têm de ser os que o caso de uso nomeia no fluxo principal ' +
    'e no wireframe, com o mesmo vocabulário do hospital. Inclua os fluxos alternativos e de ' +
    'exceção na tela (o que aparece quando o laboratório está fora do ar, quando falta dado, ' +
    'quando o envio falha). Não desenhe formulário genérico de cadastro onde o caso de uso pede ' +
    'painel, indicador, tabela ou marcação.';
  const cxi = p.getByPlaceholder(/instru|Ex:/i).first();
  if (await cxi.count()) { await cxi.focus(); await p.keyboard.type(INSTR,{delay:1});
                           console.log('instruções escritas'); await sleep(600);
                           await shot(p,'uispec-instrucoes'); }
  else console.log('!! caixa de instruções não encontrada');
