...
const PROJ = 'a1391183-f348-4a78-8773-8046b90a7676';        // "Uso do solo"
const SPEC = 'ea4bdd67';                                     // spec v2 COMPLETA 14/14 (<- req 01d24064 v1 c/ legislacao)
...
(async () => {
  ...
  await sleep(600);
  await p.screenshot({ path: `${OUT}/solo-pg-01-antes.png`, fullPage: true });
  ...
  await p.screenshot({ path: `${OUT}/solo-pg-02-resultado.png`, fullPage: true });
  const sqlTab = p.locator('button.dm-tab', { hasText: /Schema SQL/ }).first();
  if (await sqlTab.count()) { await sqlTab.click(); await sleep(1500); }
  await p.screenshot({ path: `${OUT}/solo-pg-03-schema-sql.png`, fullPage: true });
