...
(async()=>{
  ...
  await p.goto(`${BASE}/project/${PROJ}/yaml-generation`,{waitUntil:'networkidle'});
  await sleep(3000);
  // aba Tasks YAML
  ...
  else { await p.screenshot({path:`${OUT}/tasksyaml-0-modal.png`,fullPage:true}); }
  await p.screenshot({path:`${OUT}/tasksyaml-1-selected.png`,fullPage:true});
  // Gerar tasks.yaml
  ...
  await p.screenshot({path:`${OUT}/tasksyaml-2-generating.png`,fullPage:true});
  ...
  await sleep(3000);
  await p.screenshot({path:`${OUT}/tasksyaml-3-done.png`,fullPage:true});
