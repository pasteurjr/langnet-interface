...
def review_petri_net(petri_json: str, tasks_yaml: str = "", agents_yaml: str = "") -> str:
    ...
    try:
        ...
        prompt = (
            ...
            "quebra em execução.\n"
            "3a. Cada lugar traz `output_data` (o que a tarefa ENTREGA, tirado do contrato dela) e "
            "`origem_das_entradas` (de onde vem cada entrada: `LUGAR.campo` quando um lugar "
            "anterior produz, ou 'entrada externa' quando ninguém produz). JULGUE cada 'entrada "
            "externa': é mesmo digitada por uma pessoa na tela (ex.: e-mail, senha, filtro de "
            "relatório), ou é um ELO QUE FICOU SOLTO — um dado que uma tarefa anterior deveria "
            "entregar e não entrega? Quando for elo solto, diga qual tarefa anterior tem de passar "
            "a declarar aquele campo na saída.\n"
