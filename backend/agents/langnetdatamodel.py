...
def review_petri_net(petri_json: str, tasks_yaml: str = "", agents_yaml: str = "") -> str:
    ...
    try:
        ...
        prompt = (
            ...
            "CONFIRA, NESTA ORDEM:\n"
            "1. NOMES: cada transição traz `tipo`: \"tarefa\" ou \"controle\". As de CONTROLE "
            "(início, bifurcação, junção, fim) são a costura da rede e NÃO devem ter tarefa — não "
            "as aponte como divergência. Para as de TAREFA: "
            ...
            "quebra em execução.\n"
            "3a. Cada lugar traz `output_data` (o que a tarefa ENTREGA, tirado do contrato dela), "
            "`repassa` (o que CHEGA nele vindo de tras e segue adiante — na execucao cada lugar "
            "junta o que os anteriores produziram e passa o pacote inteiro para o proximo) e "
            "`origem_das_entradas` (de onde vem cada entrada: `LUGAR.campo`, com a marca "
            "'repassado pelo caminho' quando o produtor nao e o lugar imediatamente anterior, "
            "ou 'entrada externa' quando ninguem produz). NAO aponte como defeito um dado que "
            "vem marcado como repassado: ele existe e atravessa o caminho. JULGUE cada 'entrada "
            "externa': e mesmo digitada por uma pessoa na tela (ex.: e-mail, senha, filtro de "
            "relatorio), ou e um ELO QUE FICOU SOLTO — um dado que uma tarefa anterior deveria "
            "entregar e nao entrega? Quando for elo solto, diga qual tarefa anterior tem de "
            "passar a declarar aquele campo na saida.\n"
